  ...
  if (!acornTypeScript) {
    ...
    let tokenIsLiteralPropertyName = Set.prototype.has.bind(
      ...
      tokenIsIdentifier = Set.prototype.has.bind(
        new Set([...Object.values(tsKwTokenType), tokTypes.name])
      ),
...
var skipWhiteSpaceToLineBreak = new RegExp(
  ...
  "(?=(" + // Capture the whitespace and comments that should be skipped inside
    ...
    "))\\1" + // Look-ahead for either line terminator, start of multi-line comment,
    // or end of string.
    /(?=[\n\r\u2028\u2029]|\/\*(?!.*?\*\/)|$)/.source,
  "y"
  // sticky
...
function tsPlugin(options) {
  ...
  return function (Parser) {
    ...
    class TypeScriptParser extends Parser {
      ...
      isAbstractClass() {
        return (
          this.ts_isContextual(tokTypes2.abstract) &&
      ...
      isUnparsedContextual(nameStart, name) {
        ...
        if (this.input.slice(nameStart, nameEnd) === name) {
          ...
          return !(
            isIdentifierChar(nextCh) || // check if `nextCh is between 0xd800 - 0xdbff,
      ...
      parseParenAndDistinguishExpression(canBeArrow, forInit) {
        ...
        if (this.ecmaVersion >= 6) {
          ...
          while (this.type !== tt.parenR) {
            ...
            if (
              allowTrailingComma &&
              this.afterTrailingComma(tt.parenR, true)
            ) {
              lastIsComma = true;
              break;
